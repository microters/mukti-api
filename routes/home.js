...
// helper function: ensure data is an array
...
// helper function: process uploaded files
...
/**
 * GET / - Homepage data fetch endpoint
 */
...
/**
 * POST / - new homepage data creation endpoint
 */
...
// REMOVE THE GENERIC ABOUT IMAGES ENDPOINT - COMMENTED OUT
// createImageUploadEndpoint('/uploadAboutImages', 'aboutImages', 'aboutSection', 'images');

// ADD NEW CUSTOM ABOUT IMAGES ENDPOINT
router.post('/uploadAboutImages', upload.array('aboutImages', 4), async (req, res) => {
  try {
    const language = req.query.language || req.body.language || 'en';
    console.log(`Uploading aboutImages for language: ${language}`);
    
    const files = req.files || [];
    
    if (files.length === 0) {
      return res.status(400).json({ message: 'No aboutImages uploaded' });
    }
    
    console.log(`Received ${files.length} files for about section`);
    ...
    if (!homepage || !homepage.aboutSection) {
      return res.status(404).json({ message: 'aboutSection not found' });
    }
    
    const currentTranslations = homepage.aboutSection.translations || {};
    const currentData = currentTranslations[language] || {};
    ...
    // Create update data, preserving other fields
    const updateData = {
      ...currentData,
      images: filePaths
    };
    
    // Update directly using Prisma
    ...
    res.status(200).json({ 
      message: `aboutImages updated successfully for language: ${language}`,
      imagePaths: filePaths
    });
    
  } catch (error) {
    console.error('Error updating aboutImages:', error);
    res.status(500).json({ 
      error: 'Internal Server Error', 
      details: error.message 
    });
